    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        if (options.mode === 'enhance_4x' || options.mode === 'advanced_4x') {
          throw new Error(
            'Enhancement timed out. 4× upscaling on CPU can take several minutes for large images. ' +
            'Please try with a smaller image or use 2× enhancement for faster processing.'
          )
        }
